...
def run_partial_observable_bfs(game, vision_range=1):
    """
    BFS trong môi trường chỉ nhìn thấy một phần (partial observable).
    vision_range: bán kính nhìn (số ô agent có thể quan sát).
    """
    game.alg_name = "BFS (Partial Observable)"
    ...
    # Bản đồ đã biết (-1 = chưa biết, 0 = đường, 1 = tường)
    known_maze = [[-1 for _ in row] for row in game.maze]
    game.known_maze = known_maze
    game.visible_cells = []
    ...
    while queue and game.is_running:
        ...
        # Thêm các node kề vào queue (nếu ô đã "nhìn thấy" và là đường)
        for dx, dy in [(0,1),(1,0),(0,-1),(-1,0)]:
            new_x, new_y = x + dx, y + dy
            if (0 <= new_x < len(game.maze) and 
                0 <= new_y < len(game.maze[0]) and
                ...
                known_maze[new_x][new_y] == 0 and
                (new_x, new_y) not in visited_set):
                queue.append((new_x, new_y, current_path + [(x, y)]))
